      ...
      <div className="bg-white border-b border-slate-200 px-6 py-4">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-2xl font-bold text-slate-900">บอร์ดงาน (Kanban)</h1>
              <p className="text-sm text-slate-500">{filteredTasks.length} งานในระบบ</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="flex bg-slate-100 rounded-lg p-1">
              <button
                ...
                className={`p-2 rounded ${viewMode === 'board' ? 'bg-white shadow-sm' : 'hover:bg-slate-200'}`}
                title="Kanban Board"
              >
                <LayoutGrid size={18} />
              </button>
              ...
              <button
                ...
                className={`p-2 rounded ${viewMode === 'calendar' ? 'bg-white shadow-sm' : 'hover:bg-slate-200'}`}
                title="Calendar View"
              >
        ...
        <div className="flex items-center gap-4">
          ...
          <button
            ...
            className={`flex items-center gap-2 px-3 py-2 rounded-lg border ${showFilters ? 'bg-blue-50 border-blue-200 text-blue-600' : 'border-slate-200 hover:bg-slate-50'}`}
          >
            <Filter size={18} />
            <span>ตัวกรอง</span>
...
const KanbanColumn: React.FC<KanbanColumnProps> = ({
  ...
  return (
    <div
      className={`flex flex-col w-80 bg-slate-100 rounded-xl ${isDragTarget ? 'ring-2 ring-blue-400 ring-opacity-50' : ''}`}
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-slate-200">
        <div className="flex items-center gap-2">
          ...
          />
          <span className="font-medium text-slate-700">{TASK_STATUS_LABELS[status]}</span>
      ...
      {!isCollapsed && (
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          ...
          <button
            onClick={onQuickAdd}
            className="w-full flex items-center gap-2 px-3 py-2 text-slate-500 hover:text-slate-700 hover:bg-slate-200 rounded-lg transition-colors"
          >
            <Plus size={16} />
    ...
    >
      ...
      {task.tags.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          ...
          {task.tags.length > 3 && (
            <span className="px-2 py-0.5 text-xs text-slate-400">+{task.tags.length - 3}</span>
          )}
        </div>
      )}
      ...
      <div className="flex items-center justify-between mt-3 pt-2 border-t border-slate-100">
        {assignee ? (
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-slate-600 truncate max-w-[100px]">{assignee.name}</span>
          </div>
        ) : (
        ...
        {tasks.map((task) => {
          ...
          return (
            <div
              key={task.id}
              onClick={() => onTaskClick(task)}
              className="grid grid-cols-12 gap-4 px-4 py-3 hover:bg-slate-50 cursor-pointer items-center"
            >
              {/* Task */}
              <div className="col-span-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  {task.clientName && (
                    <div className="text-sm text-slate-500">{task.clientName}</div>
                  )}
                </div>
              </div>
      ...
      <div className="grid grid-cols-7 gap-1">
        {days.map((day, index) => {
          ...
          return (
            <div
              key={index}
              className={`h-24 p-1 rounded-lg border ${isToday ? 'border-blue-400 bg-blue-50' : 'border-slate-100'}`}
            >
              ...
              <div className="space-y-0.5 overflow-y-auto max-h-14">
                ...
                {dayTasks.length > 3 && (
                  <div className="text-xs text-slate-500 pl-1">+{dayTasks.length - 3} อื่นๆ</div>
                )}
              </div>
            </div>
          );
      ...
      <div className="bg-white rounded-xl shadow-xl w-full max-w-lg mx-4">
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex items-center justify-between px-6 py-4 border-b">
            <h2 className="text-lg font-semibold text-slate-900">สร้างงานใหม่</h2>
            <button
              type="button"
              onClick={onClose}
              className="text-slate-400 hover:text-slate-600"
          ...
          <div className="px-6 py-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">มอบหมายให้</label>
                <select
                  value={assignedTo}
